...
function extractHandoffLocally(messages) {
  ...
  for (const message of messages) {
    ...
    const ownerMatch = original.match(/([A-Z][a-zA-Z]+)\s+(is working on|owns|will handle|will fix|to handle)\s+(.+)/);
    if (ownerMatch) {
...
export async function extractHandoff(messages, options = {}) {
  ...
  try {
    ...
    const isEmpty = Object.values(normalized).every(
      (value) => Array.isArray(value) && value.length === 0
    );

    return isEmpty ? localFallback : normalized;
  } catch (error) {
